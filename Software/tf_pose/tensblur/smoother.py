...
class Smoother(object):
    ...
    def setup(self):
        self.feed('data').conv(name='smoothing')            #data对应的字典值为self.tensor_heatMat_up

    def get_unique_name(self, prefix):
        ident = sum(t.startswith(prefix) for t, _ in self.layers.items()) + 1
        return '%s_%d' % (prefix, ident)

    def feed(self, *args):       #在参数名之前使用一个星号，就是让函数接受任意多的位置参数，所有参数组成一个元组args
        assert len(args) != 0
        self.terminals = []
    ...
    def gauss_kernel(self, kernlen=21, nsig=3, channels=1):
        ...
        out_filter = np.repeat(out_filter, channels, axis = 2)   #复制channel次数，沿第三维度叠加
        return out_filter
    ...
    def conv(self,
        ...
        if self.heat_map_size != 0:
            ...
            c_i = input.get_shape().as_list()[3]   #返回的是各个维度的size，最后[3]代表第4个维度的size
