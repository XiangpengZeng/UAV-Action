...
from PreProcesses import body_rotation, bone_length, extract_AS, extract_data_label, extract_feature, count_zeros, feature_only_diff, one_hot_labeling
...
myKeepProb = 0.4
myHiddenSize = 100
view_subject = 'Zeng_global_local'
...
win_size = [120, 105, 90, 66, 42, 25]
#stride = [100, 100, 100, 50, 45, 25]
stride = [80, 80, 80, 50, 45, 5]
#start_time = [1, 5, 10, 1, 5, 2]       #Dl  offset
start_time = [1, 3, 5, 1, 5, 2]       #Dl  offset
...
class MP_runner(object):
    ...
    def __init__(self, is_training, config, labels):
        ...
        self.class_size = class_size = config.class_size    # 11
        ...
        with tf.device(sw_0):
            with tf.name_scope('%s_%d' % ('mGPU', 0)) as scope:
                ...
                self._cost_L = tf.reduce_sum(cross_entropy_l / 3 ) / batch_size             #zeng
        ...
        #             print(self.tvars_M)
        #             self.grad_before_sum_M = tf.gradients(self._cost_M, self.tvars_M)          #含M的要去掉，zeng
        ...
        with tf.device(sw_2):
            with tf.name_scope('%s_%d' % ('mGPU', 2)) as scope:
                ...
                self._cost_S = tf.reduce_sum(cross_entropy_s / 3) / batch_size  #zeng
                if is_training:
        ...
        with tf.device(sw_3):
            with tf.name_scope('%s_%d' % ('mGPU', 3)) as scope:
                ...
                self.m_M = m_M = TS_Motion_LSTM_Top(is_training, config, labels, self.input_data, self.targets, size)   #zeng
                logits_motion, cross_entropy_motion, state_motion = m_M.get_inference()                                 #zeng
                ...
                self._cost_motion = tf.reduce_sum(cross_entropy_motion / 3) / batch_size       #除以4是说明后面的损失函数按1：1：1：1的形式加权  zeng
                if is_training:
        ...
        with tf.device(sw_3):
            ...
            #self._cost = self._cost_L + self._cost_M + self._cost_S        #损失函数要改，他这里为什么只用了三个损失函数？
            self._cost = self._cost_L + self._cost_S + self._cost_motion    #zeng


            if not is_training:
                with tf.name_scope("Accuracy") as scope:
                    #real_logits = (logits_0 + logits_1 + logits_2 + logits_motion) / 4
                    real_logits = (logits_0  + logits_2 + logits_motion) / 3                                    #zeng
                    ...
                    self.pred_labels = tf.argmax(real_logits, 1)                                                #argmax返回下标
                ...
                with tf.name_scope("Accuracy") as scope:
                    ...
                    real_logits = (logits_l + logits_s + logits_motion) / 3
                    self._correct_prediction = tf.equal(tf.argmax(real_logits, 1), tf.argmax(self.targets, 1))
                    self.given_labels = tf.argmax(self.targets, 1)
            ...
            with tf.device(gradient_device[3]):
                with tf.name_scope("train") as scope:
                    with tf.name_scope("Merging_Gradient"):
                        ...
                        #print("M : ", len(self.grad_before_sum_M))                        zeng
                        ...
                        self.grad_after_sum = self.grad_before_sum_L  + self.grad_before_sum_S + self.grad_before_sum_motion       #zeng
                        #print("L+M+S : ", len(self.grad_after_sum))
                        print("L+S+Motion : ", len(self.grad_after_sum))
    ...
    def mp_init(self, sess):
        ...
        self.m_M.init_all_var(sess)    #zeng
    ...
    @property
    def initial_state_Motion(self):
        return self.m_M.initial_state        #zeng
    ...
    @property
    def logits_motion(self):
        return self._logits_motion
    ...
    @property
    def cost_Motion(self):
        return self._cost_motion      #zeng
...
class TS_Short_LSTM_Top(object):
    ...
    def get_inference(self):
        return self.logits, self.cross_entropy, self.final_state          #这是调用了函数
...
class TS_LSTM_Long_0(object):
    def __init__(self, is_training, config, input_data):
                ...
                with tf.variable_scope(LSTM_path):
                    if time_step < start_time_0 + win_step * stride_0:
                        ...
                        distance = (
                                       ...
                                       start_time_0 + 1)
                        (cell_output, state_0[win_step]) = cell(distance * 100, state_0[win_step])   # 按照顺序向cell输入文本数据
                        self.outputs_0[win_step].append(cell_output)
                        #-----------------------------------------------------------------------------------------
                    else:
                        cell_output = tf.zeros([batch_size, size])
        ...
        for win_step in range(num_LSTMs_0):
            output_0.append([])
            output_0[win_step] = tf.reshape(tf.concat(self.outputs_0[win_step], 1), [-1, size])   #-1代表自动计算维度
        ...
        with tf.variable_scope("Dep_Con_0"):
            ...
            concat_output_real_0 = tf.reduce_sum(input_0, 2)             #用于计算张量tensor沿着某一维度的和，可以在求和后降维
...
def run_epoch(session, m, data, label, eval_op, verbose=False, is_training=True):
    ...
    session.run(m.initial_state_Motion)
    ...
    for step, (x, y) in enumerate(Action_input.MSR_iterator(data, label, m.batch_size, m.feature_size,
        ...
        # cost_L, cost_M, cost_S, cost, accuracy, state, p_l, g_l, _ = session.run(
        #     [m.cost_L, m.cost_M, m.cost_S, m.cost, m.accuracy, m.final_state, m.pred_labels,
        #      m.given_labels, eval_op],
        #     {m.input_data: x,
        #      m.targets: y})

        cost_L, cost_S, cost_Motion, cost, accuracy, state, p_l, g_l, _ = session.run(
            [m.cost_L, m.cost_S, m.cost_Motion, m.cost, m.accuracy, m.final_state, m.pred_labels,
             m.given_labels, eval_op],
            {m.input_data: x,
             m.targets: y})                  #zeng
    ...
    return costs_L, costs_S, costs_Motion, costs, accuracys / sumsums, p_ls, g_ls      #zeng

# def run_epoch(session, m, data, label, eval_op, verbose=False, is_training=True):            #zeng
#     """Runs the model on the given data."""
#     epoch_size = ((len(data) // m.batch_size) - 1) // m.num_steps
#     # start_time = time.time()
...
def test_action():
    config = get_config()
    config.class_size = 6    #10
    config.feature_size = 28   #60
    config.input_size = 28     #60
    config.hidden_size = myHiddenSize
    config.keep_prob = myKeepProb

    eval_config = get_config()
    eval_config.class_size = 6
    eval_config.feature_size = 28        #60为固定的20个点3个坐标
    eval_config.input_size = 28
    eval_config.hidden_size = myHiddenSize
    eval_config.keep_prob = myKeepProb

    DATA_PATH = os.path.join('action_csv')
    #DATA_PATH = "/home/v4r/Desktop/Zeng/action_recognition/action_csv"
    # train_set = [1, 2]
    # test_set = [3]
    train_set = [1, 2, 3]             #zeng      for s00---s10
    test_set = [4]
    ...
    #print("train.shape:",train_sklt0.shape)
    print(DATA_PATH)
    ...
    MAX_LENGTH = 0
    print("len",len(train_sklt0))
    for batchNo in range(len(train_sklt0)):              #len(train_sklt0)为所有训练序列的数量（三维列表）
        if len(train_sklt0[batchNo]) > MAX_LENGTH:
            MAX_LENGTH = len(train_sklt0[batchNo])       #train_sklt0[batchNo]为二维列表，代表每个csv，长度为所有序列中时间长度最长的（201数据）
        else:
            pass

    for batchNo in range(len(test_sklt0)):               #总之MAX_LENGTH为数据集中最长的序列
        if len(test_sklt0[batchNo]) > MAX_LENGTH:
            MAX_LENGTH = len(test_sklt0[batchNo])
        else:
            pass

    print("MAX_LENGTH:",MAX_LENGTH)           #!!!!num_steps在这里赋值为201！！！my dataset = 123
    ...
    feature_train = body_rotation(train_sklt1)                     #本来是三维列表，经过变换后成为三维数组（numpy）!!!
    feature_test = body_rotation(test_sklt1)
    AS_train_label = one_hot_labeling(train_label0, config)        #二维列表
    ...
    config.batch_size = np.int32(len(feature_train) / BatchDivider)  ### batch_modifier，BatchDivider自定义
    eval_config.batch_size = np.int32(len(feature_test))
    config.num_steps = np.int32(len(feature_train[0]))               #201
    ...
    csv_suffix = strftime("_%Y%m%d_%H%M.csv", localtime())        #时间函数
    ...
    with tf.Graph().as_default(), tf.Session(config=sessConfig) as session:
        with tf.device('/cpu:0'):
            initializer = tf.random_uniform_initializer(-config.init_scale,
                                                        config.init_scale)      #均匀随机初始化，指定最大值，最小值，前面为负数，后面为正数
            ...
            #init = tf.global_variables_initializer()  # TF ver 0.11 #zeng
            # init = tf.global_variables_initializer() #TF ver 0.12
            #session.run(init)      #zeng

            saver = tf.train.Saver(tf.global_variables())   #保存和加载模型     !!!

            saver.restore(session, "./TrialSaver/Zeng_global_local/20201031_total/NTU_Zeng_global_local.ckpt-11300")     #zeng
            ...
            for i in range(config.max_max_epoch):
                ...
                # train_cost_L, train_cost_S, train_cost_Motion, train_cost, train_accuracy, tr_p_l, tr_g_l = run_epoch(
                ...
                #     print("Train Cost: %.6f Cost_L: %.6f Cost_S: %.6f Cost_Motion: %.6f" % (
                #         train_cost, train_cost_L, train_cost_S, train_cost_Motion))          #zeng
                #     stt_loop = time.time()
                #     print("\n")
                ...
                if i % 100 == 0:
                    print("--------------------------2----------------------------------")
                    test_cost_L, test_cost_S, test_cost_Motion, test_cost, test_accuracy, te_p_l, te_g_l = run_epoch(session,
                                                                                                                mtest,
                                                                                                                feature_test,
                                                                                                                AS_test_label,
                                                                                                                tf.no_op(),
