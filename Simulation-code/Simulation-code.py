...
_now = datetime.now()
BatchDivider = 10    
myKeepProb = 0.4         # Dropout
myHiddenSize = 112      #隐藏层神经元个数，和输入特征维度保持一致   28*4
    ...
    def __init__(self, is_training, config, labels):
        ...
        with tf.device(sw_3):
            with tf.name_scope('%s_%d' % ('mGPU', 3)) as scope:
                ...
                if is_training:
                    temp_tvars_angle = tf.trainable_variables()   #查看变量
...
class Long_Term_0(object):
    def __init__(self, is_training, config, input_data):
        ...
        for time_step in range(num_steps-1, start_time_0-1, -1):
            # end_time_0 = start_time_0 + (num_LSTMs_0 - 1) * stride_0 + win_size_0   
        ...
        for win_step in range(num_LSTMs_0 * 2):
            output_0.append([])
            output_0[win_step] = tf.reshape(tf.concat(self.outputs_0[win_step], 1), [-1, num_steps - start_time_0, size])   
...
def run_epoch(session, m, data, cos_data, label, eval_op, verbose=False, is_training=True):
    ...
    for step, (x, y, z) in enumerate(Action_input.MSR_iterator(data,cos_data, label, m.batch_size, m.feature_size,
...
def main(_):
    ...
    DATA_PATH_train = os.path.join('Normal')   # 训练集
    DATA_PATH_test = os.path.join('abNormal')  # 测试集
    ...
    train_sklt0, train_label0 = Action_input.read(DATA_PATH_train, train_set, config)
    test_sklt0, test_label0 = Action_input.read(DATA_PATH_test, test_set, eval_config)


    MAX_LENGTH = 0
    for batchNo in range(len(train_sklt0)):              #len(train_sklt0)为所有训练序列的数量（三维列表）
        if len(train_sklt0[batchNo]) > MAX_LENGTH:
            MAX_LENGTH = len(train_sklt0[batchNo])       #train_sklt0[batchNo]为二维列表，代表每个csv，长度为所有序列中时间长度最长的
        else:
            pass

    for batchNo in range(len(test_sklt0)):               # MAX_LENGTH为数据集中最长的序列
    ...
    #坐标变换。变原点
    feature_train = body_rotation(train_sklt1)                   #本来是三维列表，经过变换后成为三维数组
    feature_train_cos = body_rotation_cos(train_sklt1)           # cosine，计算余弦相似度
    feature_test = body_rotation(test_sklt1)
    feature_test_cos = body_rotation_cos(test_sklt1)

    
    #-------------------------------------扩展数据-----------------------------------------
    feature_train = np.concatenate((feature_train,feature_train,feature_train,feature_train), axis=2)
    feature_train_cos = np.concatenate((feature_train_cos,feature_train_cos,feature_train_cos,feature_train_cos),axis=2)
    feature_test = np.concatenate((feature_test,feature_test,feature_test,feature_test),axis=2)
    feature_test_cos = np.concatenate((feature_test_cos,feature_test_cos,feature_test_cos,feature_test_cos),axis=2)
    ...
    print("feature_train_cos.shape:",feature_train_cos.shape)
    ...
    with tf.Graph().as_default(), tf.Session(config=sessConfig) as session:
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = Sim_runner(is_training=True, config=config, labels=AS_train_label)
            print("\nTraining Model Established!\n")

            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mtest = Sim_runner(is_training=False, config=eval_config, labels=AS_test_label)
            ...
            for i in range(config.max_max_epoch):
                ...
                if i % 20 == 0 or (i + 1) == config.max_max_epoch:
