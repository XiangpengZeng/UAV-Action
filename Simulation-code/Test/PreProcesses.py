...
def one_hot_labeling(original_label, config):
    new_label = np.zeros([len(original_label), config.class_size])   #二维列表
...
def cos_similarity(x1,y1,x2,y2):
    ...
    if a_norm == 0 or b_norm == 0:
        return float(1) if a.all() == b.all() else float(0)   # 都是零向量则设为1
    cos_sim = np.dot(a,b)/(a_norm * b_norm)
    return cos_sim

def body_rotation_cos(feature_train):                       #每一帧都有原点
    feature_hc_train = np.zeros(feature_train.shape)
    ...
    for batchNo in range(feature_train.shape[0]):
        for frmNo in range(feature_train.shape[1]):
            if (np.linalg.norm(feature_train[batchNo][frmNo]) != 0):
                new_origin = feature_train[batchNo][frmNo][2:4]        #坐标转换，针对每行数据设计新原点（每行20个点），自己选14个点，此处原点：脖子

                for nodeNo in range(np.int32(feature_train.shape[2] / 2)):                  #多少个点的二维数据
                    # for test 新的坐标值（脖子为原点）
                    feature_temp[batchNo][frmNo][2 * nodeNo:2 * (nodeNo + 1)] \
                        = feature_train[batchNo][frmNo][2 * nodeNo:2 * (nodeNo + 1)] - new_origin
                for nodeNo_1 in range(np.int32(2),np.int32(feature_train.shape[2] / 2)):   #前面还是0
...
def feature_only_diff(data, maxValue, config):
    new_data = np.zeros([len(data), maxValue, config.input_size])    #这个三维列表各个维度相同，而原来的不同
    ...
    for batch_step in range(len(data)):                #每个csv序列文件
        ...
        #list赋值给numpy，维度一样时是可以的，这里每个序列的前面部分用来补0了
        new_data[batch_step][0:len(data[batch_step])] = norl_data  #顺序填充
        del norl_data
